...
/** 1207
 * This file is generated automatically on 2023-06-01 11:11:42
 */
#include <string.h>
#include <stdio.h>
#include <stdlib.h>
...
static unsigned long int number_request = 0;
 ...
 #define PROTO_ATTS_COUNT_17 1

/** 862
 * Proto_atts for rule 17
 */

 static proto_attribute_t proto_atts_17[ PROTO_ATTS_COUNT_17 ] = {{.proto = "http2", .proto_id = 700, .att = "header_method", .att_id = 2, .data_type = 0, .dpi_type = 1}};
/** 874
 ...
 */
 ...
 static mmt_array_t excluded_filter_17[ 3 ] = { {.elements_count = 0, .data = NULL}, 
	 {//event_1
		 .elements_count = 0,
		 .data = NULL
	 },
 ...
 */
static _msg_t_17 _m17;
 static void _allocate_msg_t_17( const char* proto, const char* att, uint16_t index ){
	 if( strcmp( proto, "http2" ) == 0 && strcmp( att, "header_method" ) == 0 ){ _m17._http2_header_method = index; return; }
 }
/** 92
 * Rule 17, event 1
  * Modify stream id
 */
static inline int g_17_1( const message_t *msg, const fsm_t *fsm ){
	 if( unlikely( msg == NULL || fsm == NULL )) return 0;
	 const message_t *his_msg;
	 const void *data;/* 57 */

	 data = get_element_data_message_t( msg, _m17._http2_header_method );
	 double _http2_header_method = 0;
	 if (data != NULL)  _http2_header_method = *(double*) data;

	 return (_http2_header_method == 130);
 }
 ...
 * Rule 17, event 2
  * Nothing
 */
static inline int g_17_2( const message_t *msg, const fsm_t *fsm ){
	 if( unlikely( msg == NULL || fsm == NULL )) return 0;
	 const message_t *his_msg;
	 const void *data;/* 57 */
...
static const rule_version_info_t version = {.created_date=1685610702, .hash = "6d0f9d6", .number="0.0.6", .index=600, .dpi="1.7.8 (68bd7d93)"};
	  ...
	  static const rule_info_t rules[] = (rule_info_t[]){
		 {
			 ...
			 .excluded_filter  = excluded_filter_17,
			 .create_instance  = &create_new_fsm_17,
			 .hash_message     = &_allocate_msg_t_17,
			 .version          = &version,
		 }
	 };
	 *rules_arr = rules;

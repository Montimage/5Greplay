...
static void em_forward1(
 ...
 #define PROTO_ATTS_COUNT_16 1

/** 862
 * Proto_atts for rule 16
 */

 static proto_attribute_t proto_atts_16[ PROTO_ATTS_COUNT_16 ] = {{.proto = "http2", .proto_id = 700, .att = "header_method", .att_id = 2, .data_type = 0, .dpi_type = 1}};
/** 874
 ...
 */
 ...
 static mmt_array_t excluded_filter_16[ 3 ] = { {.elements_count = 0, .data = NULL}, 
	 {//event_1
		 .elements_count = 0,
		 .data = NULL
	 },
 ...
 */
static _msg_t_16 _m16;
 static void _allocate_msg_t_16( const char* proto, const char* att, uint16_t index ){
	 if( strcmp( proto, "http2" ) == 0 && strcmp( att, "header_method" ) == 0 ){ _m16._http2_header_method = index; return; }
 }
/** 92
 * Rule 16, event 1
  * GET forward 
 */
static inline int g_16_1( const message_t *msg, const fsm_t *fsm ){
	 if( unlikely( msg == NULL || fsm == NULL )) return 0;
	 const message_t *his_msg;
	 const void *data;/* 57 */

	 data = get_element_data_message_t( msg, _m16._http2_header_method );
 ...
 * Rule 16, event 2
  * 
 */
static inline int g_16_2( const message_t *msg, const fsm_t *fsm ){
